...
wifi (){
  ...
  wifi_strength=$(nmcli -terse -fields SIGNAL,ACTIVE device wifi | awk --field-separator ':' '{if($2=="yes")print$1}')
  # wifi_strength=49
  # wifi_state_connection="unavailable"
  ...
  elif [ $wifi_state_connection = "unavailable" ]; then
    Icon="N/A"
